...
export default function detectVibrato(data: FrequencyRecord[]): boolean {
    ...
    for (let i = 1; i < data.length; i++) {
        if (Math.round(data[i].frequency / 2) === Math.round(data[previousData].frequency / 2)) continue;

        const change = data[i].frequency > data[previousData].frequency ? '>' : '<';
        if (opChangePoints.length === 0) {
            opChangePoints.push({
                ...data[i],
                interval: data[i].timestamp - data[previousData].timestamp,
                change,
            });
        } else {
