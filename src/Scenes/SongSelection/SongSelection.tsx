...
export default function SongSelection({ onSongSelected }: Props) {
    ...
    const songList = useQuery<SongPreview[]>('songList', () => fetch('./songs/index.json').then(response => response.json()));

    if (!songList.data) return <>Loading</>;

    return <SongSelectionContainer>
        <FocusedSong songPreview={songList.data[focusedSong]} onPlay={() => onSongSelected(songList.data[focusedSong])} />
        <SongListContainer>
            {songList.data.map((song, index) => (
                <SongListEntry key={song.file} onClick={() => setFocusedSong(index)} video={song.video}>
                    ...
                    <SongListEntryDetailsArtist>{song.artist}</SongListEntryDetailsArtist>{song.title}
                    </SongListEntryDetails>
                </SongListEntry>
            ))}
        </SongListContainer>
...
const SongListEntry = styled.div<{ video: string }>(props => `
    width: 260px;
    height: ${260 / 16 * 9}px;
    cursor: pointer;
