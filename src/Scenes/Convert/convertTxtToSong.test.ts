...
const notesToText = (notes: Note[]) => notes
...
const generateSongTxt = (trackSections: Section[][], data: Partial<Song> = {}) => {
    ...
    trackSections.forEach(([first, ...sections]) => {
        ...
        sections.forEach(section => lines.push(
            isNotesSection(section) ? `- ${section.start}\n${notesToText(section.notes)}` : `- ${section.start} ${section.end}`
        ))
    });
