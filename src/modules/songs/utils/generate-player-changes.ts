...
import { getFirstNoteStartFromSections, getLastNoteEndFromSections } from './notes-selectors';
...
export function generatePlayerChangesForTrack(track: SongTrack, song: Song) {
  const CHANGES_COUNT = 9;
  const beatLength = getSongBeatLength(song);

  if (track.sections.length < CHANGES_COUNT) return [];
...
export default function generatePlayerChanges(song: Song): number[][] {
  return song.tracks.map((track) => generatePlayerChangesForTrack(track, song));
}
