...
function SkipOutro({ onSongEnd, isEnabled, onOpenPauseMenu }: Props) {
  ...
  return !mobilePhoneMode && canSkip ? (
    <KeyboardNavContext value={register}>
      <Menu.HelpText
        {...skipNav}
        className={`z-scene-hint pointer-events-none fixed bottom-[15rem] w-full p-2 text-center text-2xl transition-all duration-500 [text-shadow:0_0_2rem_black] ${shouldBeVisible ? 'scale-100 opacity-100' : 'scale-0 opacity-0'} `}
