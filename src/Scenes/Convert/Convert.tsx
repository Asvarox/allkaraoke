...
const formatMsec = (msec: number) => {
    ...
    return <Pre>{String(minutes).padStart(2, '0')}:{String(seconds).padStart(2, '0')}.{String(miliseconds).padStart(3, '0')}</Pre>;
}
...
export default function Convert(props: Props) {
    ...
    const searchForVideo = () => {
        window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(conversionResult?.artist + ' ' + conversionResult?.title)}`, '_blank');
    }

    return (
    <Container>
        {conversionResult && conversionResult.video.length < 15 && !!(conversionResult.tracks[0].sections[0] as NotesSection).notes.length && (
        <Preview>
            <Player song={conversionResult} showControls autoplay={false} width={700} height={700 / 16 * 9} ref={player} onTimeUpdate={setCurrentTime} />
            <Editor>
                <EditorRow>
                    Playback speed:
                    <InputGroup>
                    {[0.25, 0.5, 0.75, 1, 1.25, 1.5, 2].map(speed => (
                        <InputGroupButton key={speed} onClick={() => player.current?.setPlaybackSpeed(speed)}>{speed}</InputGroupButton>
                    ))}
                    </InputGroup>
                </EditorRow>
                <EditorRow>
                    ...
                    <InputGroup>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime - 10000) / 1000)}>10000</InputGroupButton>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime - 1000) / 1000)}>1000</InputGroupButton>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime - 500) / 1000)}>-500</InputGroupButton>
                        <div style={{ flex: 1 }}><Pre>{currentTime.toFixed(0)}</Pre></div>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime + 500) / 1000)}>+500</InputGroupButton>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime + 1000) / 1000)}>+1000</InputGroupButton>
                        <InputGroupButton onClick={() => player.current?.seekTo((currentTime + 10000) / 1000)}>+10000</InputGroupButton>
                    </InputGroup>
                ...
                <EditorRow>
                    ...
                    <InputGroup>
                        <InputGroupButton onClick={() => setGapShift(shift => shift - 1000)}>-1000</InputGroupButton>
                        <InputGroupButton onClick={() => setGapShift(shift => shift - 500)}>-500</InputGroupButton>
                        <InputGroupButton onClick={() => setGapShift(shift => shift - 50)}>-50</InputGroupButton>
                        <InputGroupInput type="text" value={gapShift} onChange={e => setGapShift(+e.target.value)} placeholder="Gap shift" />
                        <InputGroupButton onClick={() => setGapShift(shift => shift + 50)}>+50</InputGroupButton>
                        <InputGroupButton onClick={() => setGapShift(shift => shift + 500)}>+500</InputGroupButton>
                        <InputGroupButton onClick={() => setGapShift(shift => shift + 1000)}>+1000</InputGroupButton>
                    </InputGroup>
                ...
                <EditorRow>
                    ...
                    <InputGroup>
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift - 10)}>-10</InputGroupButton>
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift - 5)}>-5</InputGroupButton>
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift - 1)}>-1</InputGroupButton>
                        <InputGroupInput type="text" value={videoGapShift} onChange={e => setVideoGapShift(+e.target.value)} placeholder="Gap shift" />
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift + 1)}>+1</InputGroupButton>
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift + 5)}>+5</InputGroupButton>
                        <InputGroupButton onClick={() => setVideoGapShift(shift => shift + 10)}>+10</InputGroupButton>
                    </InputGroup>
                ...
                {!!lastNote && !!beatLength && (
                    <EditorRow>
                        Last note end: {formatMsec((lastNote.start + lastNote.length) * beatLength + conversionResult.gap)}
                    </EditorRow>
                )}
                <EditorRow>
                    <InputGroup>
                        <span>Desired last note end</span>
                        <InputGroupInput type="text" value={desiredLastNoteEnd} onChange={e => setDesiredLastNoteEnd(+e.target.value)} placeholder="Desired last note end" />
                    </InputGroup>
        ...
        <Converter>
            <div style={{ flex: 1 }}>
                ...
                <InputGroup>
                    <InputGroupInput type="text" value={videoLink} onChange={e => setVideoLink(e.target.value)} placeholder="Link to Youtube Video" />
                    <InputGroupButton onClick={searchForVideo}>
                        Search for video
                    </InputGroupButton>
                </InputGroup>
                <Input type="text" value={author} onChange={e => setAuthor(e.target.value)} placeholder="Author name" />
                <Input type="text" value={authorUrl} onChange={e => setAuthorUrl(e.target.value)} placeholder="Author URL" />
                <TxtInput onChange={e => setTxtInput(fixDiacritics(e.target.value))} value={txtInput} />
            </div>
            <div style={{ flex: 1 }}>
                {conversionResult && (
                    <a
                        href={`data:application/json;charset=utf-8,${encodeURIComponent(JSON.stringify(conversionResult, undefined, 2))}`}
                ...
                <br/>
                <JsonOutput disabled value={conversionResult ? JSON.stringify(conversionResult, undefined, 2) : error} />
            </div>
        </Converter>
    </Container>);
}
